...
from noodle.entities import Direction, Point, Action
from noodle.game import SnakeGame, GameState
...
class Agent:
    ...
    def compute_reward(self, state, game: SnakeGame) -> int:
        ...
        distance_to_fruit = ((snake_head.x - fruit_position.x) ** 2 + (snake_head.y - fruit_position.y) ** 2) ** 0.5
        ...
        distance = ((snake_head.x - fruit_position.x) ** 2 + (snake_head.y - fruit_position.y) ** 2) ** 0.5
        
        # Set reward based on the distance
        reward = -distance  # Negative distance to encourage getting closer to the fruit
    ...
    def get_action(self, state):
        if random.randint(0, 100) < (self.total_games * self.epsilon) - self.n_games:
            # Explore
...
def train(WIDTH, HEIGHT, CELL_SIZE, FPS):
    ...
    while True:
        ...
        agent.train_short_memory(state_current, action, reward, state_new, game_state)
